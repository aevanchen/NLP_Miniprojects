...
def read_input(task):
	...
	path = Path("data")
	train_f = path / ("train_seq_len_20.txt")
	test_f = path / ("test_seq_len_20.txt")
	...
	if task == "copy":
		...
		train_outputs = [list(map(lambda k:str(k),(sorted(list(map(lambda x: str(x),seq)))))) for seq in train_inputs]
		test_outputs = [list(map(lambda k:str(k),(sorted(list(map(lambda x: str(x),seq)))))) for seq in test_inputs]
...
if __name__ == '__main__':
	...
	m = model(vocab_size = len(forward_dict), method=method, hidden_dim = 128)
